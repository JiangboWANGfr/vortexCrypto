...
FALLBACK = {
    ...
    "c2w4t16.combined":   (277466, 469244),
}
...
# ------------------------------------------------------------- table 4 ----
# (row label, build, parent build). dALM is against the crypto-free core;
# +% is the increase over the parent -- the S1 build a tier extends -- so a
# pair row reads the percentage the pair itself adds (the figure the prose
# quotes: 13.5%, 3.4%, 50.8%). The S1 rows and Combined extend the core.
NOC = "c2w4t16.nocrypto"
t4 = [("crypto-free core",             NOC,                  None),
      ("+ AES-GCM S1",                 "c2w4t16.base",       NOC),
      ("+ ChaCha-Poly S1",             "c2w4t16.chacha-s1",  NOC),
      ("+ ChaCha S3\\,/\\,4 pair",     "c2w4t16.chacha-s3f", "c2w4t16.chacha-s1"),
      ("+ ChaCha S3\\,/\\,16 pair",    "c2w4t16",            "c2w4t16.chacha-s1"),
      ("\\quad stateless ablation",    "c2w4t16.arx16",      "c2w4t16.chacha-s1"),
      ("+ AES S3\\,/\\,4 pair",        "c2w4t16.sg4",        "c2w4t16.base"),
      ("+ ChaCha S2 engines",          "c2w4t16.chacha-s2p", "c2w4t16.chacha-s1"),
      ("+ AES S2 engines",             "c2w4t16.s2p",        "c2w4t16.base"),
      ("Combined HARE (deployable)",   "c2w4t16.combined",   NOC)]
t4rows = []
for name, b, parent in t4:
    a, r = AREA[b]
    if parent is None:
        dcore = pct = "---"
    else:
        dcore = f"+{com(a-ZERO)}"
        step = a - AREA[parent][0]
        pct = f"{100*step/AREA[parent][0]:+.1f}\\%"
    t4rows.append(f"{name} & {com(a)} & {com(r)} & {dcore} & {pct} \\\\")
