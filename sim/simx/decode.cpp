...
Instr::Ptr Decoder::decode(uint32_t code, uint64_t uuid) {
 ...
 || defined(VX_CFG_EXT_SYM_CHACHA_ENABLE)
  ...
  case Opcode::EXT3: {
    ...
    // poly4.step.sg16 rd, rs1(h in lanes 0..4, r in 5..9), rs2(m) -- one
    // aligned sixteen-lane subgroup absorbs a whole 64-byte ChaCha block. Two
    // five-limb values fit one sixteen-lane register with six lanes to spare,
    // so this is R-type and 2R1W; funct7 is reserved.
    if (funct3 == 0x7 && funct7 == 0x0) {
      instr->set_fu_type(FUType::AUTH);
  ...
  case Opcode::EXT2: {
    ...
    // chacha.arx.sg16 rd, rs1, line -- funct3 3, funct7[2:0] the line.
    if (funct3 == 0x3 && (funct7 & 0x78) == 0) {
      instr->set_fu_type(FUType::SYM);
      instr->set_op_type(SymType::CHA_DR16);
      instr->set_dest_reg(rd, RegType::Integer);
      instr->set_src_reg(0, rs1, RegType::Integer);
      IntrSymArgs a{}; a.shamt = funct7 & 0x7;
      instr->set_args(a);
      break;
    }
#endif
