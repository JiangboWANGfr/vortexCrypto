...
inline void chacha20_block(const uint32_t k[8], uint32_t n0, uint32_t n1,
  ...
  out[2] = x[2] + 0x79622d32; out[3] = x[3] + 0x6b206574;
  for (int i = 0; i < 8; ++i) {
    out[4 + i] = x[4 + i] + k[i];
  }
  out[12] = x[12] + counter;
...
__kernel void chacha_poly_sw(kernel_arg_t* __UNIFORM__ arg) {
  ...
  for (uint32_t msg = blockIdx.x * blockDim.x + threadIdx.x; msg < num_msgs;
    ...
    for (uint32_t b = 0; b < blocks; ++b) {
      ...
      // per ChaCha20 block, so nothing is written and read back.
      for (int q = 0; q < 4; ++q) {
        const uint32_t c0 = pb[4 * q + 0] ^ ks[4 * q + 0];
        const uint32_t c1 = pb[4 * q + 1] ^ ks[4 * q + 1];
        const uint32_t c2 = pb[4 * q + 2] ^ ks[4 * q + 2];
        const uint32_t c3 = pb[4 * q + 3] ^ ks[4 * q + 3];
        cb[4 * q + 0] = c0;
        cb[4 * q + 1] = c1;
        cb[4 * q + 2] = c2;
        cb[4 * q + 3] = c3;
