// Host for the torn-quad probe. See kernel.cpp for what it does and why.
//
// The expected outcome is a REJECTION, not a number. Run it through
// ci/crypto_torn_quad.sh, which asserts that both models stop.
#include <vortex2.h>
#include <cstdio>
#include <cstdlib>
#include <cstring>
...
int main(int argc, char** argv) {
  ...
  if (num_threads < 4) {
    std::printf("SKIPPED: needs at least 4 threads per warp, has %lu\n",
  ...
  if (vx_module_get_kernel(mod, "torn_quad", &kern) != 0) {
    std::printf("SKIPPED: kernel not built (needs SYM_CHACHA_SG4)\n");
    vx_device_release(dev);
    return 0;
  }
  ...
  // untouched -- which is how the first run of this probe reported that the
  // guard had not fired when the kernel had simply never run.
  ...
  std::printf("issuing chadd.sg4 on quads masked 0111 (%u lanes)\n", lanes);
  std::fflush(stdout);
  ...
  // stale neighbour was picked up.
  std::printf("NOT REJECTED -- the torn-quad guard did not fire\n");
